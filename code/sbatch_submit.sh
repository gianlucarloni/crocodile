...
NNODES=2 #4 #4,3, or 1 #total number of nodes
NGPUSPERNODE=3 #This directive specifies the number of GPUs you want to reserve per node. In your case, you have 4 GPUs per node, so requesting 4 allows your DDP training process to utilize all available GPUs on the allocated node.
...
NCPUSPERTASK=8 #32 #While each DDP process manages a single GPU, it can still benefit from multiple CPU cores for data loading, preprocessing, and other operations alongside training.
